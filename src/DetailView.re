...
[@react.component]
let make =
    (~resource: resource, ~id: string, ~viewState=View, ~initialEntity=?, ()) => {
  let (state, setState) =
    React.useState(() =>
      Belt.Option.getWithDefault(initialEntity, initialState)
    );
  ...
  let fetchEntity = () => {
    ...
    |> Js.Promise.then_(result => {
         switch (result) {
         | Result.Ok(entity) => setState(_ => LoadSuccess(entity))
  ...
  React.useEffect1(
    () => {
      ...
      Some(() => Js.log("cleanup Effect detailview"));
    },
    [|resource.name|],
  );
           ...
           |> Array.map(
                ...
                field => {
                  ...
                  printRow(
                    ...
                    | Some(endpoint) =>
                      <Link
                        href={"/" ++ endpoint ++ "/" ++ fvalue} selected=false>
                        {str(fvalue)}
                      </Link>
                    | None => str(fvalue)
                    },
  ...
  <div id="entity">
    <button onClick={_ => fetchEntity()}>
      {str("Fetch: " ++ resource.title ++ "/" ++ id)}
    </button>
    {switch (state) {
